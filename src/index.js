...
const channels = [
  {
    name: 'Первый канал', icon: 'first.svg', program: [
  ...
  },
  {
    ...
    name: 'AMEDIA PREMIUM', icon: 'amedia.svg', program: [
    ...
    <article class="movie" id="${item.id}">
      <a href="#">
        <div class="description">
          ...
          <img src="img/movie/fade.svg" alt="" class="gray">
          <p>${item.desc}</p>
        </div>
        <h2>${item.content}</h2>
      </a>
...
function updateTv() {
  ...
  let component = channels.reduce((acc, item) => {
    acc += `<article class="channel">
            ...
            <div class="schedule">
              <h1>${item.name}</h1>
              <ul>`;
    item.program.forEach(item =>
      acc += `<li><span class="time">${item.time}</span>${item.show}</li>`);
    acc += `</ul> </div> </article>`;
    return acc;
  }, '');
...
$(".close").on('click', (() => document.querySelector("#bg").style.display = "none"));
